const SELECTORS = {
  ...
  themeToggleIcon: "[data-theme-toggle-icon]",
};
...
const THEME_STORAGE_KEY = "joaquin-berro-theme";
...
function initializeTheme() {
  ...
  const themeToApply = storedTheme || (prefersLight ? THEMES.LIGHT : THEMES.DARK);
  applyDocumentTheme(themeToApply);
}
...
function initThemeToggle() {
  ...
  const label = document.querySelector(SELECTORS.themeToggleLabel);
  const icon = document.querySelector(SELECTORS.themeToggleIcon);
  const prefersScheme = window.matchMedia?.("(prefers-color-scheme: light)") || null;
  ...
  const handleSchemeChange = (event) => {
    ...
    setTheme(event.matches ? THEMES.LIGHT : THEMES.DARK, { persist: false });
  };

  if (prefersScheme) {
    if (typeof prefersScheme.addEventListener === "function") {
      prefersScheme.addEventListener("change", handleSchemeChange);
    } else if (typeof prefersScheme.addListener === "function") {
      prefersScheme.addListener(handleSchemeChange);
    }
  }
}
